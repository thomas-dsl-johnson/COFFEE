...
parameter ALIGNMENT_ABITS=2;                            // Request address alignment (address bits)
parameter COUNTER_WIDTH=6;                              // This parameter is not set by lsu_top, so is left at it's default value.  It seems like this should be derived from the INPUTFIFOSIZE parameter?
...
parameter ALMOST_EMPTY_THRESH=1;                        // almost empty threshold, o_almost_empty asserts if the number of available thread is LESS THAN this number, only applies if USE_STALL_LATENCY=1
parameter ALLOW_HIGH_SPEED_FIFO_USAGE=1;                // choice of hld_fifo style, 0 = mid speed fifo, 1 = high speed fifo
parameter USE_WRITE_ACK=0;                              // Use the write-acknowledge signal, when disabled valids can
                                                        // exit write LSUs immediately 
                                                        // NOTE: This doesn't affect o_active, which waits
                                                        // for avm_writeack
...
wire lsu_write_acknowledged;
generate
if(USE_WRITE_ACK==1)
begin
  assign lsu_write_acknowledged = avm_writeack;
end
else
begin
  assign lsu_write_acknowledged = avm_write && !avm_waitrequest;
end
endgenerate
...
begin
  ...
  begin
      ...
      begin
          avm_writeack_dly <= lsu_write_acknowledged;
      end
  end
...
begin
  ...
  reg [COUNTER_WIDTH-1:0] mem_ack_counter; // acknowledge writes counter from the MEMORY
  wire mem_ack_incr, mem_ack_decr;
  ...
  // Note o_active needs to always wait for the writeack signal to return.
  assign occ_incr = write_accepted;
  assign occ_decr = lsu_o_valid && !lsu_i_stall;
  assign ack_incr = lsu_write_acknowledged;
  assign ack_decr = lsu_o_valid && !lsu_i_stall;
  // Track the number of input requests vs. number of memory acks
  assign mem_ack_incr = avm_writeack;
  assign mem_ack_decr = i_valid && !o_stall & !i_predicate;  // Start o_active when we accept the input
  // o_active should be high if we are waiting for an ack
  assign o_active = mem_ack_counter[COUNTER_WIDTH-1];
  ...
  begin
      ...
      begin
          ...
          mem_ack_counter <= mem_ack_counter + { {(COUNTER_WIDTH-1){!mem_ack_incr && mem_ack_decr}}, (mem_ack_incr ^ mem_ack_decr) };
          if(~sclrn[0]) begin
             occ_counter <= '0;
             ack_counter <= '0;
             mem_ack_counter <= '0;
