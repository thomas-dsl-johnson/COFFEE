...
parameter USE_WRITE_ACK=0;      // Use the write-acknowledge signal, when disabled valids can exit write LSUs immediately
                                // NOTE: This doesn't affect o_active, which waits for avm_writeack
...
acl_ll_fifo #(
  ...
  .data_out( rf_count ),
  .write( write_accepted && (!rf_empty || !lsu_write_acknowledged) ),
  .read( rf_read ),
  .empty( rf_empty ),
  .full( rf_full )
);
assign rf_read = lsu_write_acknowledged && !rf_empty;
...
generate
   if (COUNTER_WIDTH-2 > 0) begin
      assign occ_counter_update_extended = {{(COUNTER_WIDTH-2){occ_counter_update[1]}}, occ_counter_update};
   end else begin
      assign occ_counter_update_extended = occ_counter_update;
   end
endgenerate
always@(posedge clk or posedge reset)
begin
   ...
   begin
      ...
      ack_counter <= ack_counter + ({COUNTER_WIDTH{lsu_write_acknowledged}} & ( rf_empty ? or_next_counter : rf_count )) - output_acknowledged;
   end
end
...
assign o_active = (occ_counter != {COUNTER_WIDTH{1'b0}}) || (ack_counter != {COUNTER_WIDTH{1'b0}});
