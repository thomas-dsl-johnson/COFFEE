...
module const_lambda_function_cra_agent (
    ...
    // bit_enable_bottom(BITSELECT,119)
    ...
    // write_selector_start_status(MUX,257)
    ...
    // finish_counter_addr(CONSTANT,148)
    ...
    // finish_counter_NO_SHIFT_REG(REG,147)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // padded_finish_counter(BITJOIN,210)
    ...
    // join_address_cmp(BITJOIN,183)
    assign join_address_cmp_q = {NO_NAME_q, address_cmp_0_q};

    // readdata_upper_bits_mux(MUX,223)
    ...
    // readdata_lower_bits_mux(MUX,221)
    ...
    // avs_cra_readdatavalid(GPOUT,361)
    ...
    // dupName_12_NO_NAME_x(LOGICAL,327)
    ...
    // bit_enable_top_bar(BITSELECT,122)
    ...
    // global_offset_reg_upper_0_buffered(REG,164)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,163)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0_buffered(REG,158)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,157)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_13_NO_NAME_x(LOGICAL,331)
    ...
    // global_offset_reg_upper_1_buffered(REG,166)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,165)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1_buffered(REG,160)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,159)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_14_NO_NAME_x(LOGICAL,335)
    ...
    // global_offset_reg_upper_2_buffered(REG,168)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,167)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2_buffered(REG,162)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,161)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_7_NO_NAME_x(LOGICAL,299)
    ...
    // global_size_reg_lower_0_buffered(REG,173)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,172)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1_buffered(REG,175)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,174)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_8_NO_NAME_x(LOGICAL,305)
    ...
    // global_size_reg_lower_2_buffered(REG,177)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,176)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_7_address_ref_x(CONSTANT,300)
    ...
    // kernel_arg_bit_join(BITJOIN,186)
    assign kernel_arg_bit_join_q = {dupName_3_arg_bit_join_x_q, dupName_2_arg_bit_join_x_q, dupName_1_arg_bit_join_x_q, dupName_0_arg_bit_join_x_q, arg_bit_join_q};

    // pipe_reg0_kernel_arguments(REG,211)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,213)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg0_kernel_start(REG,212)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_start(REG,214)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // local_size_reg_0_buffered(REG,189)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_1_buffered(REG,191)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_2_buffered(REG,193)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_0_buffered(REG,205)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_1_buffered(REG,207)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_2_buffered(REG,209)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_NO_NAME_x(LOGICAL,293)
    ...
    // work_dim_NO_SHIFT_REG_buffered(REG,254)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG_buffered(REG,256)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,255)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
